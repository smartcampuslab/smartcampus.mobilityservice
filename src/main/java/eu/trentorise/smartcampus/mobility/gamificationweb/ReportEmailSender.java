...
public class ReportEmailSender {
	...
	public synchronized void checkNotification() throws Exception {
		for (AppInfo appInfo : appSetup.getApps()) {
			checkNotification(appInfo.getAppId());
		}
	}
	...
	public synchronized void checkNotification(String appId) throws Exception {
		...
		// ArrayList<BadgesData> allBadgeTest = getAllBadges(path);
		// try {
		...
		for (Player p : iter) {
			...
			if (p.isSendMail()) {
				...
				String unsubcribeLink = mainUrl + "/gamificationweb/unsubscribeMail/?socialId=" + encriptedId; // p.getSocialId();
				// ArrayList<State> states = null;
				List<PointConcept> states = null;
				ArrayList<Notification> notifications = null;
				ArrayList<BadgesData> someBadge = null;
				...
				try {
					...
					try {
						PlayerStatus completePlayerStatus = statusUtils.correctPlayerData(completeState, p.getId(), gameId, p.getNickname(), challUtils, mainUrl + "/gamificationweb/", 0, language);
					...
					notifications = getNotifications(urlWSNot, timestamp, appId);
				} catch (InterruptedException ie) {
					logger.error(String.format("Ws invoke sleep exception  %s", ie.getMessage()));
				}

				if (notifications != null && notifications.size() > 0) {
					ArrayList<BadgesData> allBadge = getAllBadges(path);
					...
					try {
						if (notifications != null) {
							if (states != null && states.size() > 0) {
								this.emailService.sendMailGamification(!p.isFirstMailSent(), playerName, states.get(0).getScore() + "", null, null, null, null, // health and pr point are null
								...
								this.emailService.sendMailGamification(!p.isFirstMailSent(), playerName, "0", "0", "0", null, null, actual_week, actual_week_theme, last_week, are_chall, are_prizes,
...
//		ArrayList<MailImage> standardImages = new ArrayList<MailImage>();
//
//		File greenScore = new File(path + "img/mail/green/greenLeavesbase.png");
//		File healthScore = new File(path + "img/mail/health/healthLeavesBase.png");
//		File prScore = new File(path + "img/mail/pr/prLeaves.png");
//		File footer = new File(path + "img/mail/templateMail.png");
//		File foglie03 = new File(path + "img/mail/foglie03.png");
//		File foglie04 = new File(path + "img/mail/foglie04.png");
...
//				summaryMail.add(new Summary(p.getName() + " " + p.getSurname() + ": " + p.getNickname(), (states != null) ? states.toString() : "", (notifications != null) ? notifications.toString() : ""));
				summaryMail.add(new Summary(p.getName() + " " + p.getSurname() + ": " + p.getNickname(), (states != null && !states.isEmpty()) ? Integer.toString(states.get(0).getScore()) : "",
						(notifications != null) ? notifications.toString() : ""));
			} else {
				logger.info("Mail non inviata a " + p.getNickname() + ". L'utente ha richiesto la disattivazione delle notifiche.");
		...
		for (Player p : iter) {
			...
			if (p.isSendMail()) {
				...
				try {
					...
					String unsubcribeLink = mainUrl + "/gamificationweb/rest/unsubscribeMail/?socialId=" + encriptedId; // + p.getSocialId();
					// ArrayList<State> states = null;
					List<PointConcept> states = null;
					ArrayList<Notification> notifications = null;
					List<ChallengesData> challenges = null;
					...
					try {
						...
						try {
							PlayerStatus completePlayerStatus = statusUtils.correctPlayerData(completeState, p.getId(), gameId, p.getNickname(), challUtils, mainUrl + "/gamificationweb/", 0,
									language);
							states = completePlayerStatus.getPointConcept();
							ChallengeConcept challLists = completePlayerStatus.getChallengeConcept();
							if (challLists != null) {
...
//					summaryMail.add(new Summary(p.getName() + " " + p.getSurname() + ": " + p.getNickname(), (states != null) ? states.toString() : "", (notifications != null) ? notifications.toString() : ""));
	...
	private ArrayList<BadgesData> getAllBadges(String path) throws IOException {
	...
	private ArrayList<BadgesData> checkCorrectBadges(ArrayList<BadgesData> allB, ArrayList<Notification> notifics) throws IOException {
		ArrayList<BadgesData> correctBadges = new ArrayList<BadgesData>();
	 ...
	 * @throws InterruptedException
	 */
	private ArrayList<Notification> getNotifications(String urlWS, String timestamp, String appId) throws InterruptedException {
