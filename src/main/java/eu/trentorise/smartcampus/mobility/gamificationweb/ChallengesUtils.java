...
//	private final String CHAL_DESC_1A = "Fai almeno altri TARGET viaggi MODE e avrai un bonus di BONUS punti POINT_TYPE";
//	private final String CHAL_DESC_1B = "Fai almeno TARGET km MODE e avrai un bonus di BONUS punti POINT_TYPE";
...
//	private final String CHAL_DESC_5A = "Ottieni almeno altri TARGET punti POINT_TYPE e avrai ulteriori BONUS punti POINT_TYPE in bonus";
...
//	private final String CHAL_TYPE_1A = "PERCENTTRIPS";
...
//	private final String SERVER_CHAL_ALLOWED_MODE_PANDR = "PandR";
...
//	private final String SERVER_CHAL_ALLOWED_MODE_KM = "Km";
//	
...
//	private final String CHAL_ALLOWED_MODE_PANDR = "pandr";
...
//	private final String CHAL_ALLOWED_MODE_PANDR_DIS = "pandrDistance";
//	
...
//			} else if(mobMode.compareTo(CHAL_ALLOWED_MODE_Z) == 0 || mobMode.compareTo(CHAL_ALLOWED_MODE_Z + "Distance") == 0){
//				correctDesc = (language.compareTo(ITA_LANG) == 0) ? challDescList.get(6).getDescription() : challDescList.get(6).getDescription_eng();
//			} else if(mobMode.compareTo(CHAL_ALLOWED_MODE_PANDR) == 0 || mobMode.compareTo(CHAL_ALLOWED_MODE_PANDR + "Distance") == 0){
//				correctDesc = (language.compareTo(ITA_LANG) == 0) ? challDescList.get(34).getDescription() : challDescList.get(34).getDescription_eng();
//			}
//		} else if(type.compareTo(CHAL_TYPE_1B) == 0){
//			if(mobMode.compareTo(CHAL_ALLOWED_MODE_BK) == 0 || mobMode.compareTo(CHAL_ALLOWED_MODE_BK + "Distance") == 0){
//				correctDesc = (language.compareTo(ITA_LANG) == 0) ? challDescList.get(0).getDescription() : challDescList.get(0).getDescription_eng();
//			} else if(mobMode.compareTo(CHAL_ALLOWED_MODE_BKS) == 0 || mobMode.compareTo(CHAL_ALLOWED_MODE_BKS + "Distance") == 0){
//				correctDesc = (language.compareTo(ITA_LANG) == 0) ? challDescList.get(1).getDescription() : challDescList.get(1).getDescription_eng();
...
//			correctDesc = (language.compareTo(ITA_LANG) == 0) ? challDescList.get(10).getDescription().replace("X", target).replace("[green leaves, bici, salute, impatto 0]", pointType) : challDescList.get(10).getDescription_eng().replace("X", target).replace("[green leaves, bike, health, 0 impact]", pointType);
...
//    }
	...
	public List<List> correctChallengeData(String profile, int type, String language, List<PointConcept> pointConcept, List<BadgeCollectionConcept> bcc_list) throws Exception {
    	...
    	if(profile != null && profile.compareTo("") != 0){
    		...
    		List<ChallengeConcept> challengeList = challengeManager.parse(profile);
    		...
    		if(challengeList != null && !challengeList.isEmpty()){
				for(ChallengeConcept challenge: challengeList){
					...
					if(challenge.getFields() != null){
						bonusScore = (int)challenge.getFields().getOrDefault(CHAL_FIELDS_BONUS_SCORE, 0);
						...
						target =  (int)challenge.getFields().getOrDefault(CHAL_FIELDS_TARGET,0); ///(int)((!chalFields.isNull(CHAL_FIELDS_TARGET)) ? chalFields.getDouble(CHAL_FIELDS_TARGET) : 0);
						...
						baseline = (int)challenge.getFields().getOrDefault(CHAL_FIELDS_BASELINE,0); //(!chalFields.isNull(CHAL_FIELDS_BASELINE)) ? chalFields.getInt(CHAL_FIELDS_BASELINE) : 0;
...
//						final String ch_point_type = challData.getBonusPointType();
    				...
    				tmp_chall.setChallDesc(challengeManager.fillDescription(challenge, language));
    				...
    				switch (tmp_chall.getType()) {
	    				...
	    				case CHAL_MODEL_ABSOLUTE_INC: {
		    				...
		    				tmp_chall.setChallCompleteDesc(challengeManager.fillLongDescription(challenge, "counterName", language));
	    					break;
	    				}
	    				case CHAL_MODEL_NEXT_BADGE: {
		    				...
		    				tmp_chall.setChallCompleteDesc(challengeManager.fillLongDescription(challenge, "badgeCollectionName", language));
		    				break;
	    				}
	    				...
	    				case CHAL_MODEL_POICHECKIN: {
		    				...
		    				tmp_chall.setChallCompleteDesc(challengeManager.fillLongDescription(challenge, "eventName", language));
		    				break;
	    				}
	    				case CHAL_MODEL_CLASSPOSITION: {
		    				...
		    				tmp_chall.setChallCompleteDesc(challengeManager.fillLongDescription(challenge, null, language));
		    				break;
	    				}
...
//		}
//		if(cName.contains(SERVER_CHAL_ALLOWED_MODE_PANDR)){
//			corrMobility = CHAL_ALLOWED_MODE_PANDR;
//		}
...
//	    	}
//	    	if(target == 1 && desc.contains("viaggi ")){
//	    		desc = desc.replace("viaggi ", "viaggio ");
//	    		if(desc.contains("altri 1")){
//	    			desc = desc.replace("altri 1", "1 altro");
//	    		}
//	    	}
...
//    		}
//    	}
//    	if(mode.compareTo(CHAL_ALLOWED_MODE_PANDR) == 0){
//    		if(language.compareTo(ITA_LANG) == 0){
//    			corr_mode = (type == 0) ? "utilizzando il park and ride" : "il park and ride";
//    		} else {
//    			corr_mode = (type == 0) ? "in park and ride mode" : "park and ride mode";
//    		}
//    	}
...
//    		}
//    	}
//    	if(mode.compareTo(CHAL_ALLOWED_MODE_PANDR_DIS) == 0){
//    		if(language.compareTo(ITA_LANG) == 0){
//    			corr_mode = (type == 0) ? "utilizzando il park and ride" : "il park and ride";
//    		} else {
//    			corr_mode = (type == 0) ? "in park and ride mode" : "park and ride mode";
