...
public class RoveretoPlanningPolicy implements PlanningPolicy {
		...
		for (PlanningRequest pr: originalPlanningRequests) {
			...
			if (type.equals(TType.CAR) || type.equals(TType.CARWITHPARKING)) {
				if (!allTypes.contains(TType.PARK_AND_RIDE)) {
					PlanningRequest npr = PlanningPolicyHelper.buildDefaultDerivedRequest(journeyRequest, pr, TType.PARK_AND_RIDE, 1, true, prg1a);
					result.add(npr);
					allTypes.add(TType.PARK_AND_RIDE);
				}	
				
				if (!allTypes.contains(TType.WALK)) {
					PlanningRequest npr = PlanningPolicyHelper.buildDefaultDerivedRequest(journeyRequest, pr, TType.WALK, 1, true, null);
					result.add(npr);
					allTypes.add(TType.WALK);
				}	
				
				if (!allTypes.contains(TType.TRANSIT)) {
					PlanningRequest npr = PlanningPolicyHelper.buildDefaultDerivedRequest(journeyRequest, pr, TType.TRANSIT, null, true, prg1b);
					result.add(npr);
					allTypes.add(TType.TRANSIT);
				}
				
				if (!allTypes.contains(TType.SHAREDBIKE)) {
					PlanningRequest npr = PlanningPolicyHelper.buildDefaultDerivedRequest(journeyRequest, pr, TType.SHAREDBIKE, null, true, prg1b);
					result.add(npr);
			...
			if (type.equals(TType.TRANSIT) || type.equals(TType.BUS) || type.equals(TType.TRAIN)) {
				if (!allTypes.contains(TType.TRAIN)) {
					PlanningRequest npr = PlanningPolicyHelper.buildDefaultDerivedRequest(journeyRequest, pr, TType.TRAIN, null, true, prg2);
					result.add(npr);
					allTypes.add(TType.TRAIN);
				}	
				
				if (!allTypes.contains(TType.SHAREDBIKE)) {
					PlanningRequest npr = PlanningPolicyHelper.buildDefaultDerivedRequest(journeyRequest, pr, TType.SHAREDBIKE, null, true, prg2);
					result.add(npr);
					allTypes.add(TType.SHAREDBIKE);
				}					
				
				if (!allTypes.contains(TType.WALK)) {
					PlanningRequest npr = PlanningPolicyHelper.buildDefaultDerivedRequest(journeyRequest, pr, TType.WALK, 1, true, prg2);
					result.add(npr);
					allTypes.add(TType.WALK);
				}					
			}
	...
	public List<Itinerary> filterAndSortItineraries(SingleJourney journeyRequest, List<Itinerary> itineraries) {
		...
		for (Itinerary it : itineraries) {
			...
			if (analysis.maxDistance != 0 && distance > 1000 * analysis.maxDistance) {
				toRemove.add(it);
