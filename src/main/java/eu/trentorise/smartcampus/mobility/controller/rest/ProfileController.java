...
//	private static FastDateFormat shortSdfDb = FastDateFormat.getInstance("yyyy/MM/dd");
	...
	public void generateWaypoints(String campaignId) throws Exception {
		SortedSet<String> months = findMonths(campaignId);
		...
		for (String month: months) {
			createMissingMonth(month, currentMonth, campaignId, players);
		}
	}

	private PlayerProfile convertPlayer(Player p, int year) {
		PlayerProfile pp = new PlayerProfile();
		pp.setUser_id(p.getPlayerId());
	...
	private void completeWithSurvey(PlayerProfile pp, Player p, int year) {
		...
		String gender;
		switch ((String) survey.get("gender")) {
		case "maschio":
			gender = "male";
			break;
		case "femmina":
	...
	private void createMissingMonth(String date, String currentDate, String campaignId, List<Player> players) throws Exception {
		...
		File f = new File(waypointsDir + "/waypoints_" + campaignId + "_" + suffix + ".zip");
		if (f.exists()) {
		...
		ZipEntry ze = new ZipEntry("waypoints_" + campaignId + "_" + suffix + ".json");
		fos.putNextEntry(ze);
	...
	private SortedSet<String> findMonths(String appId) {
		Criteria criteria = new Criteria("appId").is(appId).and("freeTrackingTransport").ne(null);
		Query query = new Query(criteria);
